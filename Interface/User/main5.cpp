...
int Open_Cube5X5(sf::RenderWindow &window)
{
    ...
    Cube5X5 input_Cube, check_Cube;
    freopen("../Project/Solve_Cube/test_case_5X5/testcase1.txt", "r", stdin);
    for (i=0; i<6; i++)
    {
        for (j=0; j<25; j++)
        {
            cin >> input_Cube.color[i][j];
            check_Cube.color[i][j] = input_Cube.color[i][j];
        }
    }

    Spray_Cube_5X5(cube5X5_color, input_Cube);
            ...
            if (event.type == sf::Event::KeyPressed)
            {
                     ...
                     sf::Keyboard::isKeyPressed(sf::Keyboard::RShift))
                {
                    for (i = 0; i < 6; i++) 
                    {
                        ...
                        {
                            // Rotate the corresponding face of the 5x5 cube
                            switch (mode_of_rotate)
                            {
                                case 1: input_Cube = Rotate_Small_Anti_Clockwise_5X5(input_Cube, i);
                                        input_Cube = Rotate_Big_Anti_Clockwise_5X5(input_Cube, i);
                                        break;
                                case 2: input_Cube = Rotate_Big_Anti_Clockwise_5X5(input_Cube, i);
                                        break;
                                case 3: input_Cube = Rotate_Small_Anti_Clockwise_5X5(input_Cube, i);
                                        break;
                            }
                            sound.play();
                ...
                else 
                {
                    for (i = 0; i < 6; i++) 
                    {
                        if (event.key.code == keys[i]) 
                        {
                            ...
                            switch (mode_of_rotate)
                            {
                                case 1: input_Cube = Rotate_Small_Clockwise_5X5(input_Cube, i);
                                        input_Cube = Rotate_Big_Clockwise_5X5(input_Cube, i);
                                        break;
                                case 2: input_Cube = Rotate_Big_Clockwise_5X5(input_Cube, i);
                                        break;
                                case 3: input_Cube = Rotate_Small_Clockwise_5X5(input_Cube, i);
                                        break;
                            }
                            sound.play();
                            break; // Break after the correct key is found
